package com.github.clementherve.intellijjavadependencyupdaterplugin.update;
...
/**
 * Platform tests for {@link DependencyVersionWriter}.
 */
public class DependencyVersionWriterTest extends BasePlatformTestCase {
    ...
    private Dependency findByArtifact(List<Dependency> dependencies, String artifact) {
    ...
    private String update(PsiFile file, Dependency dependency, String newVersion) {
        WriteCommandAction.runWriteCommandAction(getProject(), () ->
                DependencyVersionWriter.applyUpdateInWriteAction(getProject(), dependency, newVersion));
    ...
    public void test_update_version_that_is_a_prefix_of_another_version() {
        ...
        Dependency annotations = findByArtifact(dependencies, "jackson-annotations");
    ...
    public void test_update_longer_version_keeps_other_lines_intact() {
        ...
        Dependency core = findByArtifact(dependencies, "jackson-core");
    ...
    public void test_parsing_versions_are_exact() {
        PsiFile file = configure();
        List<Dependency> dependencies = new GradleBuildFileParser().parseDependencies(file);
    ...
    public void test_update_all_dependencies_in_one_write_action() {
        ...
        Dependency swagger = findByArtifact(dependencies, "swagger-annotations");
        Dependency jsr305 = findByArtifact(dependencies, "jsr305");
        Dependency core = findByArtifact(dependencies, "jackson-core");
        Dependency annotations = findByArtifact(dependencies, "jackson-annotations");
        Dependency databind = findByArtifact(dependencies, "jackson-databind");

        List<Dependency> reverseOrdered = dependencies.stream()
                .sorted(Comparator.comparingInt(this::offsetOf).reversed())
                .toList();

        WriteCommandAction.runWriteCommandAction(getProject(), () -> {
            for (Dependency dependency : reverseOrdered) {
                DependencyVersionWriter.applyUpdateInWriteAction(getProject(), dependency, newVersionFor(dependency));
            }
        });
        PsiDocumentManager.getInstance(getProject()).commitAllDocuments();
    ...
    public void test_version_substring_in_artifact_is_not_corrupted() {
        ...
        Dependency dependency = new GradleBuildFileParser().parseDependencies(file).getFirst();
    ...
    public void test_full_file_update_variable_version() {
        ...
        Dependency junitApi = findByArtifact(dependencies, "junit-jupiter-api");
    ...
    public void test_full_file_update_plugin_version() {
        ...
        Dependency plugin = findByArtifact(dependencies, "com.github.ben-manes.versions");
    ...
    private String newVersionFor(Dependency dependency) {
    ...
    private int offsetOf(Dependency dependency) {
